...
contract('ASCValidatorSet', (accounts) => {
  it('query basic info', async () => {
    const validatorSetInstance = await ASCValidatorSet.deployed();
...
contract('ASCValidatorSet', (accounts) => {
  it('burn', async () => {
    ...
    await govHub.updateContractAddr(ASCValidatorSet.address, SlashIndicator.address, SystemReward.address, LightClient.address, MockTokenHub.address, RelayerIncentivize.address, RelayerHub.address, GovHub.address, TokenManager.address, crossChain.address, crossChain.address);
...
function jailRlpEncode(consensusAddrList,feeAddrList, ascFeeAddrList) {
  let pkg = [];
  pkg.push(0x01);
...
function validatorUpdateRlpEncode(consensusAddrList,feeAddrList, ascFeeAddrList) {
  let pkg = [];
  pkg.push(0x00);
  let n = consensusAddrList.length;
  let vals = [];
