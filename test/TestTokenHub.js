...
contract('TokenHub', (accounts) => {
    ...
    it('Relayer transfer from ABC to AXC', async () => {
        const tokenHub = await TokenHub.deployed();
        const abcToken = await ABCToken.deployed();
        const crossChain = await CrossChain.deployed();
    ...
    it('Expired transfer from ABC to AXC', async () => {
        const tokenHub = await TokenHub.deployed();
        const abcToken = await ABCToken.deployed();
        const crossChain = await CrossChain.deployed();
    ...
    it('Relayer AXC transfer from ABC to AXC', async () => {
        const tokenHub = await TokenHub.deployed();
        const crossChain = await CrossChain.deployed();
        const relayer = accounts[1];
    ...
    it('Transfer from AXC to ABC', async () => {
        const crossChain = await CrossChain.deployed();
        const tokenHub = await TokenHub.deployed();
        const abcToken = await ABCToken.deployed();
    ...
    it('enable mirror and sync channel', async () => {
        ...
        await govHub.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address, TokenHub.address, RelayerIncentivize.address, RelayerHub.address, GovHub.address, TokenManager.address, CrossChain.address, CrossChain.address);
        ...
        await govHub.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address, TokenHub.address, RelayerIncentivize.address, RelayerHub.address, GovHub.address, TokenManager.address, accounts[8], CrossChain.address);
    });
