...
const AXCValidatorSetTool = artifacts.require("tool/AXCValidatorSetTool");
...
module.exports = function(deployer, network, accounts) {
  ...
  deployer.deploy(AXCValidatorSetTool);
  ...
  deployer.deploy(TypesToBytes).then(function() {
    ...
    slashInstance.init();
    deployer.link(TypesToBytes, AXCValidatorSet);
    deployer.link(BytesToTypes, AXCValidatorSet);
    deployer.link(Memory, AXCValidatorSet);
    deployer.link(BytesLib, AXCValidatorSet);
    deployer.link(CmnPkg, AXCValidatorSet);
    deployer.link(RLPDecode, AXCValidatorSet);
    ...
    return deployer.deploy(AXCValidatorSet).then(function (validatorInstance) {
      validatorInstance.init();
      stakingInstance.init();
      relayerIncentivizeInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address);
      tendermintLightClientInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address);
      tokenHubInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address);
      tokenMgrInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address);
      govHubInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, accounts[8], Staking.address);
      slashInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address);
      validatorInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,MockTokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, accounts[8], Staking.address);
      relayerHubInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address);
      crossChainInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address);
      stakingInstance.updateContractAddr(AXCValidatorSet.address, SlashIndicator.address, SystemReward.address, MockLightClient.address,TokenHub.address,RelayerIncentivize.address,RelayerHub.address,GovHub.address, TokenManager.address, CrossChain.address, Staking.address)
