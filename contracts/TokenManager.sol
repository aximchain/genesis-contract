...
contract TokenManager is System, IApplication, IParamSubscriber {
  ...
  uint256 public constant MAX_GAS_FOR_TRANSFER_AXC = 10000;
  ...
  function decodeBindSynPackage(
    bytes memory msgBytes
  ) internal pure returns (BindSynPackage memory, bool) {
  ...
  function handleBindSynPackage(bytes memory msgBytes) internal returns (bytes memory) {
    ...
    if (bindSynPkg.packageType == BIND_PACKAGE) {
      ...
      address contractAddr = ITokenHub(TOKEN_HUB_ADDR).getContractAddrByBEP2Symbol(
  ...
  function encodeReactBindSynPackage(
    ReactBindSynPackage memory reactBindSynPackage
  ...
  function approveBind(
    ...
    require(
      contractAddr == bindSynPkg.contractAddr,
      "contact address doesn't equal to the contract address in bind request"
    );
    require(
      IBEP20(contractAddr).getOwner() == msg.sender,
      'only bep20 owner can approve this bind request'
    );
    uint256 tokenHubBalance = IBEP20(contractAddr).balanceOf(TOKEN_HUB_ADDR);
    require(
      IBEP20(contractAddr).allowance(msg.sender, address(this)).add(tokenHubBalance) >=
        lockedAmount,
    ...
    require(
      relayFee >= miniRelayFee && relayFee % TEN_DECIMALS == 0,
      'relayFee must be N * 1e10 and greater than miniRelayFee'
    );

    uint32 verifyCode = verifyBindParameters(bindSynPkg, contractAddr);
    if (verifyCode == CODE_OK) {
      IBEP20(contractAddr).transferFrom(
        msg.sender,
        TOKEN_HUB_ADDR,
        lockedAmount.sub(tokenHubBalance)
      );
      ITokenHub(TOKEN_HUB_ADDR).bindToken(
        bindSynPkg.bep2TokenSymbol,
        bindSynPkg.contractAddr,
        bindSynPkg.bep20Decimals
      );
    ...
    ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
      BIND_CHANNELID,
      encodeReactBindSynPackage(reactBindSynPackage),
      relayFee.div(TEN_DECIMALS)
    );
    return true;
  }
  ...
  function encodeMirrorSynPackage(
  ...
  function decodeMirrorSynPackage(
    bytes memory msgBytes
  ) internal pure returns (MirrorSynPackage memory, bool) {
    MirrorSynPackage memory mirrorSynPackage;
    RLPDecode.Iterator memory iter = msgBytes.toRLPItem().iterator();
    bool success = false;
    uint256 idx = 0;
  ...
  function decodeMirrorAckPackage(
    bytes memory msgBytes
  ) internal pure returns (MirrorAckPackage memory, bool) {
  ...
  function mirror(address bep20Addr, uint64 expireTime) public payable returns (bool) {
    require(
      ITokenHub(TOKEN_HUB_ADDR).getBep2SymbolByContractAddr(bep20Addr) == bytes32(0x00),
      'already bound'
    );
    require(!mirrorPendingRecord[bep20Addr], 'mirror pending');
    uint256 miniRelayFee = ITokenHub(TOKEN_HUB_ADDR).getMiniRelayFee();
    require(
      msg.value % TEN_DECIMALS == 0 && msg.value >= mirrorFee.add(miniRelayFee),
      'msg.value must be N * 1e10 and greater than sum of miniRelayFee and mirrorFee'
    );
    require(
      expireTime >= block.timestamp + 120 && expireTime <= block.timestamp + 86400,
      'expireTime must be two minutes later and one day earlier'
    );
    uint8 decimals = IBEP20(bep20Addr).decimals();
    uint256 totalSupply = IBEP20(bep20Addr).totalSupply();
    require(
      convertToBep2Amount(totalSupply, decimals) <= MAX_BEP2_TOTAL_SUPPLY,
      'too large total supply'
    );
    ...
    require(
      symbolBytes.length >= MINIMUM_BEP20_SYMBOL_LEN &&
        symbolBytes.length <= MAXIMUM_BEP20_SYMBOL_LEN,
      'symbol length must be in [2,8]'
    );
    for (uint8 i = 0; i < symbolBytes.length; i++) {
      require(
        (symbolBytes[i] >= 'A' && symbolBytes[i] <= 'Z') ||
          (symbolBytes[i] >= 'a' && symbolBytes[i] <= 'z') ||
          (symbolBytes[i] >= '0' && symbolBytes[i] <= '9'),
        'symbol should only contain alphabet and number'
      );
    }
    ...
    ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
      ...
      msg.value.sub(mirrorFee).div(TEN_DECIMALS)
    );
    return true;
  }

  function handleMirrorAckPackage(bytes memory msgBytes) internal {
    (MirrorAckPackage memory mirrorAckPackage, bool decodeSuccess) = decodeMirrorAckPackage(
      msgBytes
    );
    ...
    if (mirrorAckPackage.errorCode == CODE_OK) {
      ...
      ITokenHub(TOKEN_HUB_ADDR).bindToken(
        mirrorAckPackage.bep2Symbol,
        mirrorAckPackage.bep20Addr,
        mirrorAckPackage.bep20Decimals
      );
      boundByMirror[mirrorAckPackage.bep20Addr] = true;
      emit mirrorSuccess(mirrorAckPackage.bep20Addr, mirrorAckPackage.bep2Symbol);
      return;
    } else {
      (bool success, ) = mirrorAckPackage.mirrorSender.call{
        gas: MAX_GAS_FOR_TRANSFER_AXC,
        value: mirrorAckPackage.mirrorFee
      }('');
      if (!success) {
  ...
  function handleMirrorFailAckPackage(bytes memory msgBytes) internal {
    (MirrorSynPackage memory mirrorSynPackage, bool decodeSuccess) = decodeMirrorSynPackage(
      msgBytes
    );
    require(decodeSuccess, 'unrecognized package');
    mirrorPendingRecord[mirrorSynPackage.bep20Addr] = false;
    (bool success, ) = mirrorSynPackage.mirrorSender.call{
      gas: MAX_GAS_FOR_TRANSFER_AXC,
      value: mirrorSynPackage.mirrorFee.mul(TEN_DECIMALS)
    }('');
    if (!success) {
  ...
  function decodeSyncSynPackage(
    bytes memory msgBytes
  ) internal pure returns (SyncSynPackage memory, bool) {
    SyncSynPackage memory syncSynPackage;
    RLPDecode.Iterator memory iter = msgBytes.toRLPItem().iterator();
    bool success = false;
    uint256 idx = 0;
  ...
  function decodeSyncAckPackage(
    bytes memory msgBytes
  ) internal pure returns (SyncAckPackage memory, bool) {
  ...
  function sync(address bep20Addr, uint64 expireTime) public payable returns (bool) {
    ...
    uint256 miniRelayFee = ITokenHub(TOKEN_HUB_ADDR).getMiniRelayFee();
    require(
      msg.value % TEN_DECIMALS == 0 && msg.value >= syncFee.add(miniRelayFee),
      'msg.value must be N * 1e10 and no less sum of miniRelayFee and syncFee'
    );
    require(
      expireTime >= block.timestamp + 120 && expireTime <= block.timestamp + 86400,
      'expireTime must be two minutes later and one day earlier'
    ...
    ICrossChain(CROSS_CHAIN_CONTRACT_ADDR).sendSynPackage(
      SYNC_CHANNELID,
      encodeSyncSynPackage(syncSynPackage),
      msg.value.sub(syncFee).div(TEN_DECIMALS)
    );
    return true;
  }
  ...
  function handleSyncAckPackage(bytes memory msgBytes) internal {
    ...
    (bool success, ) = syncAckPackage.syncSender.call{
      gas: MAX_GAS_FOR_TRANSFER_AXC,
      value: syncAckPackage.syncFee
    }('');
    if (!success) {
  ...
  function handleSyncFailAckPackage(bytes memory msgBytes) internal {
    ...
    require(decodeSuccess, 'unrecognized package');
    (bool success, ) = syncSynPackage.syncSender.call{
      gas: MAX_GAS_FOR_TRANSFER_AXC,
      value: syncSynPackage.syncFee.mul(TEN_DECIMALS)
    }('');
    if (!success) {
  ...
  function updateParam(string calldata key, bytes calldata value) external override onlyGov {
    ...
    if (bytes32Key == bytes32(0x6d6972726f724665650000000000000000000000000000000000000000000000)) {
      // mirrorFee
      ...
      bytes32Key == bytes32(0x73796e6346656500000000000000000000000000000000000000000000000000)
    ) {
      // syncFee
      uint256 newSyncFee;
      assembly {
  ...
  function verifyBindParameters(
    ...
    if (
      ITokenHub(TOKEN_HUB_ADDR).getContractAddrByBEP2Symbol(bindSynPkg.bep2TokenSymbol) !=
      address(0x00) ||
      ITokenHub(TOKEN_HUB_ADDR).getBep2SymbolByContractAddr(bindSynPkg.contractAddr) !=
      bytes32(0x00)
    ) {
  ...
  function checkSymbol(
    string memory bep20Symbol,
    bytes32 bep2TokenSymbol
  ) internal pure returns (bool) {
    bytes memory bep20SymbolBytes = bytes(bep20Symbol);
    if (
      bep20SymbolBytes.length > MAXIMUM_BEP20_SYMBOL_LEN ||
      bep20SymbolBytes.length < MINIMUM_BEP20_SYMBOL_LEN
    ) {
      return false;
    }
