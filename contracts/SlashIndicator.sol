...
contract SlashIndicator is ISlashIndicator,System,IParamSubscriber, IApplication{
  ...
  uint256 public constant ASC_RELAYER_REWARD = 1e16;
  ...
  // The ASC validators assign proper values for `misdemeanorThreshold` and `felonyThreshold` through governance.
  // The proper values depends on ASC network's tolerance for continuous missing blocks.
  ...
  function slash(address validator) external onlyCoinbase onlyInit oncePerBlock onlyZeroGasPrice{
    if (!IASCValidatorSet(VALIDATOR_CONTRACT_ADDR).isCurrentValidator(validator)) {
      return;
    }
    ...
    if (indicator.count % felonyThreshold == 0) {
      indicator.count = 0;
      IASCValidatorSet(VALIDATOR_CONTRACT_ADDR).felony(validator);
      ...
      IASCValidatorSet(VALIDATOR_CONTRACT_ADDR).misdemeanor(validator);
    }
  ...
  function encodeSlashPackage(address valAddr) internal view returns (bytes memory) {
    ...
    elements[2] = uint256(ascChainID).encodeUint();
